...
final class ArchitectureTargetBoundaryTests: XCTestCase {
    ...
    func testProviderRuntimeCannotEscalateCredentialAccess() throws {
        let rootURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        let providersURL = rootURL.appendingPathComponent("Sources/OhMyUsage/Providers")
        for sourceFile in try swiftFiles(in: providersURL) {
            let source = try String(contentsOf: sourceFile, encoding: .utf8)
            let relative = relativePath(for: sourceFile, rootURL: rootURL)
            XCTAssertFalse(
                source.contains("SecurityCredentialReader."),
                "\(relative) must not read external Keychain credentials during Provider fetch"
            )
            XCTAssertFalse(
                source.contains("accessIntent: .interactiveImport") || source.contains("accessIntent: .authRecovery"),
                "\(relative) must keep live browser access in explicit import coordinators"
            )
            XCTAssertFalse(
                source.contains("forceRefresh ? .interactiveImport"),
                "\(relative) must not treat forceRefresh as user authorization"
            )
            XCTAssertFalse(
                source.contains("prepareSecureStoreAccess("),
                "\(relative) must not unlock the CraftMeter vault during Provider fetch"
            )
            if source.contains("keychain.saveToken(") {
                XCTAssertTrue(
                    source.contains("intent: .backgroundPersistence"),
                    "\(relative) Provider Keychain writes must explicitly remain background-only"
                )
            }
        }
    }
