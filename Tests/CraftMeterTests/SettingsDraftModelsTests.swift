...
final class SettingsDraftModelsTests: XCTestCase {
    ...
    func testNewRelaySiteDraftStateResetPreservesTemplateAndClearsTransientSelection() {
        var state = NewRelaySiteDraftState(
            ...
            testStatusVisible: true,
            browserImportInFlight: true,
            curlImportInFlight: true,
            curlImportResult: RelayCurlImportDisplayResult(
                success: false,
        ...
        XCTAssertFalse(state.testStatusVisible)
        XCTAssertFalse(state.browserImportInFlight)
        XCTAssertFalse(state.curlImportInFlight)
        XCTAssertNil(state.curlImportResult)
        XCTAssertEqual(state.templateID, "generic-newapi")
        XCTAssertNil(state.selectedPresetID)
    }

    func testNewRelaySiteDraftInvalidationClearsCurlImportResult() {
        var state = NewRelaySiteDraftState(
            curlImportResult: RelayCurlImportDisplayResult(
                success: true,
                host: "relay.example.com",
