...
final class TraeProviderTests: XCTestCase {
    ...
    func testFetchMaps401And403ToExplicitReimportGuidance() async throws {
        for statusCode in [401, 403] {
            try await assertFetchError(
                statusCode: statusCode,
                body: #"{"msg":"unauthorized"}"#,
                matcher: {
                    guard case .unauthorizedDetail(let message) = $0 else { return false }
                    return message.contains("重新导入")
                }
            )
        }
    }
    ...
    func testFetchWithSavedTokenDoesNotReadBrowserCandidates() async throws {
        ...
        let service = "CraftMeterTests-Trae-\(UUID().uuidString)"
    ...
    func testFetchDoesNotRefreshExpiredSavedTokenFromBrowser() async throws {
        let keychain = makeTestKeychain()
        let service = "CraftMeterTests-Trae-\(UUID().uuidString)"
        let account = "official/trae/cloud-ide-jwt-\(UUID().uuidString)"
        ...
        let spy = TraeBrowserCandidateSpy(
            candidatesByHost: ["trae.ai": [BrowserDetectedCredential(value: "fresh.jwt.token", source: "Chrome")]]
        )
        let provider = TraeProvider(
            descriptor: descriptor,
            session: session,
        ...
        await XCTAssertThrowsTraeProviderError { _ = try await provider.fetch(forceRefresh: true) }
        XCTAssertTrue(spy.calls.isEmpty)
        XCTAssertEqual(keychain.readToken(service: service, account: account), "expired.jwt.token")
    }

    func testFetchDoesNotUseBrowserCandidateWhenSavedTokenMissing() async {
        let keychain = makeTestKeychain()
        let service = "CraftMeterTests-Trae-\(UUID().uuidString)"
        let account = "official/trae/cloud-ide-jwt-\(UUID().uuidString)"
        var descriptor = ProviderDescriptor.defaultOfficialTrae()
        ...
        let spy = TraeBrowserCandidateSpy(
            candidatesByHost: ["trae.ai": [BrowserDetectedCredential(value: "fresh.jwt.token", source: "Arc")]]
        )
        let provider = TraeProvider(
            descriptor: descriptor,
            keychain: keychain,
        ...
        await XCTAssertThrowsTraeProviderError { _ = try await provider.fetch(forceRefresh: true) }
        XCTAssertTrue(spy.calls.isEmpty)
        XCTAssertNil(keychain.readToken(service: service, account: account))
    }
...
private func XCTAssertThrowsTraeProviderError(
