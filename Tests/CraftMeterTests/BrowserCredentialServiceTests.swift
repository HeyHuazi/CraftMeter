...
final class BrowserCredentialServiceTests: XCTestCase {
    ...
    func testBackgroundIntentDoesNotReuseCachedCookieHeader() {
        var lookupCount = 0
        let service = BrowserCredentialService(
            cookieHeaderOverride: { _ in
                lookupCount += 1
        ...
        XCTAssertEqual(
            service.detectCookieHeader(host: "relay.example.com", accessIntent: .interactiveImport)?.value,
            "session=ok"
        )
        XCTAssertNil(
            service.detectCookieHeader(host: "relay.example.com", accessIntent: .background)
        )
        XCTAssertEqual(lookupCount, 1)
    }

    func testBackgroundIntentDoesNotReuseCachedBearerOrNamedCookie() {
        var bearerLookupCount = 0
        var namedLookupCount = 0
        let service = BrowserCredentialService(
            bearerCandidatesOverride: { _ in
                bearerLookupCount += 1
                return [BrowserDetectedCredential(value: "bearer", source: "browser")]
            },
            namedCookieOverride: { _, _ in
                namedLookupCount += 1
                return BrowserDetectedCredential(value: "named=value", source: "browser")
            },
            cacheTTL: 30
        )

        XCTAssertEqual(
            service.detectBearerTokenCandidates(host: "relay.example.com", accessIntent: .interactiveImport).map(\.value),
            ["bearer"]
        )
        XCTAssertEqual(
            service.detectNamedCookie(name: "session", host: "relay.example.com", accessIntent: .interactiveImport)?.value,
            "named=value"
        )

        XCTAssertTrue(
            service.detectBearerTokenCandidates(host: "relay.example.com", accessIntent: .background).isEmpty
        )
        XCTAssertNil(
            service.detectNamedCookie(name: "session", host: "relay.example.com", accessIntent: .background)
        )
        XCTAssertEqual(bearerLookupCount, 1)
        XCTAssertEqual(namedLookupCount, 1)
    }
