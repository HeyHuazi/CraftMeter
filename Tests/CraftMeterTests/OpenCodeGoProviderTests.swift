...
final class OpenCodeGoProviderTests: XCTestCase {
    ...
    func testForceRefreshDoesNotImportOpenCodeBrowserCookie() async {
        let now = try! makeDate("2026-04-23T12:00:00Z")
        let workspaceAccount = "official/opencode-go/workspace-id-\(UUID().uuidString)"
        let cookieAccount = "official/opencode-go/auth-cookie-\(UUID().uuidString)"
        let keychain = makeTestKeychain()
        XCTAssertTrue(keychain.saveToken("wrk_test_workspace", service: KeychainService.defaultServiceName, account: workspaceAccount))
        ...
        await XCTAssertThrowsOpenCodeProviderError {
            _ = try await provider.fetch(forceRefresh: true)
        }
        XCTAssertEqual(browserSpy.detectNamedCookieCallCount, 0)
        XCTAssertEqual(browserSpy.detectCookieHeaderCallCount, 0)
        XCTAssertNil(keychain.readToken(service: KeychainService.defaultServiceName, account: cookieAccount))
    }
...
private func XCTAssertThrowsOpenCodeProviderError(
