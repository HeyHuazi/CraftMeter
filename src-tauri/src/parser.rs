// Parse ~/.claude/projects/**/*.jsonl, dedupe assistant messages by id,
// classify tool calls (user-installed MCP / Skill only), and aggregate
// into Day / Week / Month reports + a daily heatmap.
use crate::config::UserConfig;
use crate::model::*;
use crate::pricing::Pricing;
use crate::store::{RawCraftCall, RawEvent, Store};
use chrono::{DateTime, Datelike, Duration, Local, Timelike};
use std::collections::{HashMap, HashSet};
use std::sync::Mutex;

// Serializes dashboard builds so the background refresh thread and the command
// handler never touch the incremental cache files concurrently.
static BUILD_LOCK: Mutex<()> = Mutex::new(());

// One assistant API response, with config + pricing applied (derived per request
// from a RawEvent, since user config / prices / time windows can all change).
struct Event {
    ts: DateTime<Local>,
    session: String,
    model: String,
    project: String,
    input: f64,          // raw tokens, uncached new input only
    cache: f64,          // raw tokens, cache creation + read
    output: f64,         // raw tokens
    reasoning: f64,      // raw reasoning/thinking output tokens
    cost: f64,           // USD (differentiated by token type), 0 if unknown model
    priced: bool,        // whether a price was found for this model
    mcp: Vec<String>,    // user-installed server names called in this msg
    skills: Vec<String>, // user-installed skill names called in this msg
    tool: String,
    enabled_sources: Vec<String>,
    permission_mode: String,
    thinking_level: String,
    craft_calls: Vec<RawCraftCall>,
}

// Top-5 models keep the green/slate scheme; everything beyond is uniform gray.
const PALETTE: &[&str] = &["#1f9d63", "#34c27e", "#6ad0a0", "#a7e3c5", "#4b5a52"];
const OVERFLOW_GRAY: &str = "#79817b";

/// Strip a trailing "-YYYYMMDD" date suffix so dated releases merge into
/// their base model (e.g. "claude-haiku-4-5-20251001" → "claude-haiku-4-5").
const MONTHS: [&str; 12] = [
    "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec",
];

fn normalize_model(name: &str) -> String {
    if let Some(idx) = name.rfind('-') {
        let suffix = &name[idx + 1..];
        if suffix.len() == 8 && suffix.chars().all(|c| c.is_ascii_digit()) {
            return name[..idx].to_string();
        }
    }
    name.to_string()
}

fn vendor_of(model: &str) -> &'static str {
    let m = model.to_lowercase();
    if m.contains("claude") {
        "Anthropic"
    } else if m.contains("gpt") || m.contains("o1") || m.contains("o3") {
        "OpenAI"
    } else if m.contains("gemini") {
        "Google"
    } else if m.contains("llama") {
        "Local"
    } else if m.contains("glm") {
        "Zhipu"
    } else if m.contains("deepseek") {
        "DeepSeek"
    } else {
        "Other"
    }
}

#[derive(Debug, Clone, Copy, Default)]
pub struct PeriodOffsets {
    pub day: i32,
    pub week: i32,
    pub month: i32,
}

impl PeriodOffsets {
    fn clamped(self) -> Self {
        Self {
            day: self.day.min(0),
            week: self.week.min(0),
            month: self.month.min(0),
        }
    }
}

pub fn build_dashboard() -> Dashboard {
    build_dashboard_with_offsets(PeriodOffsets::default())
}

pub fn build_dashboard_with_offsets(offsets: PeriodOffsets) -> Dashboard {
    let _guard = BUILD_LOCK.lock().unwrap_or_else(|e| e.into_inner());

    let offsets = offsets.clamped();

    // 1. Ingest incrementally (full scan only on first run; afterwards just the
    //    appended lines), prune events older than the heatmap window, and persist
    //    only when something actually changed — so an idle tick doesn't rewrite
    //    the entire events.json every 30s.
    let mut store = Store::load();
    let mut dirty = store.ingest();
    // Reports/heatmap span ~26 weeks (+ prev month); 210 days leaves margin.
    let cutoff = (Local::now() - Duration::days(210)).timestamp_millis();
    if store.prune_before(cutoff) {
        dirty = true;
    }
    if dirty {
        store.save();
    }

    // 2. Aggregate: apply current config + prices, slice by current time.
    let cfg = UserConfig::load();
    // Memoized price table (cheap clone); loaded/refreshed off-thread elsewhere
    // so neither parsing nor the network runs while we hold BUILD_LOCK.
    let pricing = Pricing::shared();
    let events: Vec<Event> = store
        .events
        .iter()
        .map(|r| compute_event(r, &cfg, &pricing))
        .collect();

    let now = Local::now();
    let today = now.date_naive();

    let mut day = report_day(&events, now, offsets.day);
    let mut week = report_week(&events, now, offsets.week);
    let mut month = report_month(&events, now, offsets.month);
    let heatmap = build_heatmap(&events, today);

    // "servers"/"skills" = how many the user has *installed* (global, constant
    // across periods), not how many were called in the window.
    let installed_servers = cfg.mcp_servers.len() as u64;
    let installed_skills = cfg.skills.len() as u64;
    for r in [&mut day, &mut week, &mut month] {
        r.metrics.servers = installed_servers;
        r.metrics.skills = installed_skills;
    }

    // today's displayed tokens (M) for the tray
    let today_tokens: f64 = events
        .iter()
        .filter(|e| e.ts.date_naive() == today)
        .map(|e| (e.input + e.cache + e.output + e.reasoning) / 1e6)
        .sum();

    Dashboard {
        day,
        week,
        month,
        heatmap,
        today_tokens,
        generated_at: now.to_rfc3339(),
    }
}

/// Derive a computed Event from a stored RawEvent, applying the *current* user
/// config (MCP/Skill whitelist) and prices. This is why these aren't baked into
/// the store: installing an MCP or a price refresh applies retroactively.
fn compute_event(r: &RawEvent, cfg: &UserConfig, pricing: &Pricing) -> Event {
    let ts = DateTime::from_timestamp_millis(r.ts_ms)
        .unwrap_or_default()
        .with_timezone(&Local);
    let model = normalize_model(&r.model);
    // price lookup uses the raw (possibly dated) id, then the normalized one
    let cost_opt = pricing
        .cost(&r.model, r.in_tok, r.out_tok, r.cc, r.cr)
        .or_else(|| pricing.cost(&model, r.in_tok, r.out_tok, r.cc, r.cr));
    let mcp = r
        .mcp
        .iter()
        .filter(|s| cfg.is_user_mcp(s))
        .cloned()
        .collect();
    let skills = r
        .skills
        .iter()
        .filter(|s| cfg.is_user_skill(s))
        .map(|s| s.rsplit(':').next().unwrap_or(s).to_string())
        .collect();
    Event {
        ts,
        session: r.session.clone(),
        model,
        project: if r.project.is_empty() {
            "unknown".to_string()
        } else {
            r.project.clone()
        },
        input: r.in_tok,
        cache: r.cc + r.cr,
        output: r.out_tok,
        reasoning: r.reasoning_tok,
        cost: cost_opt.unwrap_or(0.0),
        priced: cost_opt.is_some(),
        mcp,
        skills,
        tool: r.tool.clone(),
        enabled_sources: r.enabled_sources.clone(),
        permission_mode: r.permission_mode.clone(),
        thinking_level: r.thinking_level.clone(),
        craft_calls: r.craft_calls.clone(),
    }
}

#[derive(Default)]
struct CraftToolAcc {
    display_name: String,
    category: String,
    call_count: u64,
    error_count: u64,
    completed_count: u64,
    executing_count: u64,
}

#[derive(Default)]
struct CraftCategoryAcc {
    call_count: u64,
    error_count: u64,
    completed_count: u64,
    executing_count: u64,
}

#[derive(Default)]
struct CraftFacetAcc {
    sessions: HashSet<String>,
    billable_tokens: f64,
    cost_cents: u64,
}

#[derive(Default)]
struct ClientAcc {
    input: f64,
    cache: f64,
    output: f64,
    reasoning: f64,
    cost: f64,
    requests: u64,
    sessions: HashSet<String>,
    priced: bool,
}

fn client_id(tool: &str) -> String {
    let id = tool.trim();
    if id.is_empty() {
        "unknown".to_string()
    } else {
        id.to_string()
    }
}

fn client_label(id: &str) -> String {
    match id {
        "craft-agent" => "Craft Agents".to_string(),
        "claude-code" => "Claude Code".to_string(),
        "codex" => "Codex".to_string(),
        "unknown" => "Unknown".to_string(),
        other => other
            .split(['-', '_'])
            .filter(|s| !s.is_empty())
            .map(|s| {
                let mut chars = s.chars();
                match chars.next() {
                    Some(first) => first.to_uppercase().collect::<String>() + chars.as_str(),
                    None => String::new(),
                }
            })
            .collect::<Vec<_>>()
            .join(" "),
    }
}

// ── aggregation helpers ────────────────────────────────────────────
#[derive(Default)]
struct Agg {
    input: f64,
    cache: f64,
    output: f64,
    reasoning: f64,
    cost: f64,
    requests: u64,
    sessions: HashSet<String>,
    mcp_calls: u64,
    skill_calls: u64,
    model_tok: HashMap<String, f64>,
    model_cost: HashMap<String, f64>,
    model_priced: HashMap<String, bool>,
    client_usage: HashMap<String, ClientAcc>,
    project_usage: HashMap<String, ClientAcc>,
    mcp_counts: HashMap<String, u64>,
    skill_counts: HashMap<String, u64>,
    craft_tools: HashMap<String, CraftToolAcc>,
    craft_sources: HashMap<String, CraftFacetAcc>,
    craft_categories: HashMap<String, CraftCategoryAcc>,
    craft_status: HashMap<String, u64>,
    craft_permission: HashMap<String, CraftFacetAcc>,
    craft_thinking: HashMap<String, CraftFacetAcc>,
}

impl Agg {
    fn add(&mut self, e: &Event) {
        self.input += e.input;
        self.cache += e.cache;
        self.output += e.output;
        self.reasoning += e.reasoning;
        self.cost += e.cost;
        if !e.session.is_empty() {
            self.sessions.insert(e.session.clone());
        }
        // Slash-command skill events carry no model (empty) — they're not LLM
        // requests, so they must not inflate request counts or the model split.
        if !e.model.is_empty() {
            self.requests += 1;
            // model totals keep all token types so shares sum to Total tokens
            *self.model_tok.entry(e.model.clone()).or_default() +=
                e.input + e.cache + e.output + e.reasoning;
            *self.model_cost.entry(e.model.clone()).or_default() += e.cost;
            // a model is "priced" if any of its messages had a known price
            *self.model_priced.entry(e.model.clone()).or_default() |= e.priced;
            self.add_client(e);
            self.add_project(e);
        }
        for s in &e.mcp {
            self.mcp_calls += 1;
            *self.mcp_counts.entry(s.clone()).or_default() += 1;
        }
        for s in &e.skills {
            self.skill_calls += 1;
            *self.skill_counts.entry(s.clone()).or_default() += 1;
        }
        self.add_craft(e);
    }

    fn add_client(&mut self, e: &Event) {
        let id = client_id(&e.tool);
        let client = self.client_usage.entry(id).or_default();
        client.input += e.input;
        client.cache += e.cache;
        client.output += e.output;
        client.reasoning += e.reasoning;
        client.cost += e.cost;
        client.requests += 1;
        client.priced |= e.priced;
        if !e.session.is_empty() {
            client.sessions.insert(e.session.clone());
        }
    }

    fn add_project(&mut self, e: &Event) {
        let key = if e.project.is_empty() {
            "unknown"
        } else {
            &e.project
        };
        let project = self.project_usage.entry(key.to_string()).or_default();
        project.input += e.input;
        project.cache += e.cache;
        project.output += e.output;
        project.reasoning += e.reasoning;
        project.cost += e.cost;
        project.requests += 1;
        project.priced |= e.priced;
        if !e.session.is_empty() {
            project.sessions.insert(e.session.clone());
        }
    }

    fn add_craft(&mut self, e: &Event) {
        if e.tool != "craft-agent" && e.craft_calls.is_empty() && e.enabled_sources.is_empty() {
            return;
        }
        let billable = e.input + e.output;
        let cents = (e.cost * 100.0).round() as u64;

        for call in &e.craft_calls {
            let tool = self.craft_tools.entry(call.name.clone()).or_default();
            tool.display_name = call.display_name.clone();
            tool.category = call.category.clone();
            tool.call_count += 1;
            if call.is_error {
                tool.error_count += 1;
            }
            match call.status.as_str() {
                "completed" => tool.completed_count += 1,
                "executing" => tool.executing_count += 1,
                _ => {}
            }

            let cat = self
                .craft_categories
                .entry(call.category.clone())
                .or_default();
            cat.call_count += 1;
            if call.is_error {
                cat.error_count += 1;
            }
            match call.status.as_str() {
                "completed" => cat.completed_count += 1,
                "executing" => cat.executing_count += 1,
                _ => {}
            }
            *self.craft_status.entry(call.status.clone()).or_default() += 1;
        }

        for slug in &e.enabled_sources {
            let src = self.craft_sources.entry(slug.clone()).or_default();
            src.sessions.insert(e.session.clone());
            src.billable_tokens += billable;
            src.cost_cents += cents;
        }
        if !e.permission_mode.is_empty() {
            let p = self
                .craft_permission
                .entry(e.permission_mode.clone())
                .or_default();
            p.sessions.insert(e.session.clone());
            p.billable_tokens += billable;
            p.cost_cents += cents;
        }
        if !e.thinking_level.is_empty() {
            let t = self
                .craft_thinking
                .entry(e.thinking_level.clone())
                .or_default();
            t.sessions.insert(e.session.clone());
            t.billable_tokens += billable;
            t.cost_cents += cents;
        }
    }

    fn models(&self) -> Vec<ModelStat> {
        let mut v: Vec<(String, f64, f64)> = self
            .model_tok
            .iter()
            .map(|(k, t)| (k.clone(), *t, *self.model_cost.get(k).unwrap_or(&0.0)))
            .collect();
        v.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
        v.into_iter()
            .enumerate()
            .map(|(i, (name, tok, cost))| {
                let priced = *self.model_priced.get(&name).unwrap_or(&false);
                ModelStat {
                    vendor: vendor_of(&name).to_string(),
                    tokens: (tok / 1e6 * 100.0).round() / 100.0,
                    cost: (cost * 100.0).round() / 100.0,
                    color: if i < PALETTE.len() {
                        PALETTE[i]
                    } else {
                        OVERFLOW_GRAY
                    }
                    .to_string(),
                    priced,
                    name,
                }
            })
            .collect()
    }

    fn clients(&self) -> Vec<ClientUsageStat> {
        let mut v: Vec<ClientUsageStat> = self
            .client_usage
            .iter()
            .map(|(id, a)| {
                let total = a.input + a.cache + a.output + a.reasoning;
                ClientUsageStat {
                    id: id.clone(),
                    label: client_label(id),
                    total_tokens: (total / 1e6 * 100.0).round() / 100.0,
                    input_tokens: (a.input / 1e6 * 100.0).round() / 100.0,
                    cache_tokens: (a.cache / 1e6 * 100.0).round() / 100.0,
                    output_tokens: (a.output / 1e6 * 100.0).round() / 100.0,
                    reasoning_tokens: (a.reasoning / 1e6 * 100.0).round() / 100.0,
                    cost: (a.cost * 100.0).round() / 100.0,
                    requests: a.requests,
                    sessions: a.sessions.len() as u64,
                    priced: a.priced,
                }
            })
            .collect();
        v.sort_by(|a, b| {
            b.total_tokens
                .partial_cmp(&a.total_tokens)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
        v
    }

    fn projects(&self) -> Vec<ProjectStat> {
        let mut v: Vec<ProjectStat> = self
            .project_usage
            .iter()
            .map(|(name, a)| ProjectStat {
                name: name.clone(),
                total_tokens: ((a.input + a.cache + a.output + a.reasoning) / 1e6 * 100.0).round()
                    / 100.0,
                cost: (a.cost * 100.0).round() / 100.0,
                requests: a.requests,
                sessions: a.sessions.len() as u64,
                priced: a.priced,
            })
            .collect();
        v.sort_by(|a, b| {
            b.cost
                .partial_cmp(&a.cost)
                .unwrap_or(std::cmp::Ordering::Equal)
                .then_with(|| {
                    b.total_tokens
                        .partial_cmp(&a.total_tokens)
                        .unwrap_or(std::cmp::Ordering::Equal)
                })
        });
        v
    }

    fn session_metrics(&self) -> SessionMetrics {
        SessionMetrics {
            duration_seconds: 0,
            active_seconds: 0,
            message_count: self.requests,
            user_message_count: 0,
        }
    }

    fn named(counts: &HashMap<String, u64>) -> Vec<NamedCount> {
        let mut v: Vec<NamedCount> = counts
            .iter()
            .map(|(k, c)| NamedCount {
                name: k.clone(),
                count: *c,
            })
            .collect();
        v.sort_by(|a, b| b.count.cmp(&a.count));
        v
    }

    fn craft(&self) -> CraftAgentReport {
        let mut tools: Vec<CraftToolStat> = self
            .craft_tools
            .iter()
            .map(|(name, a)| CraftToolStat {
                name: name.clone(),
                display_name: a.display_name.clone(),
                category: a.category.clone(),
                call_count: a.call_count,
                error_count: a.error_count,
                completed_count: a.completed_count,
                executing_count: a.executing_count,
            })
            .collect();
        tools.sort_by(|a, b| b.call_count.cmp(&a.call_count));

        let mut sources: Vec<CraftSourceStat> = self
            .craft_sources
            .iter()
            .map(|(slug, a)| CraftSourceStat {
                slug: slug.clone(),
                session_count: a.sessions.len() as u64,
                total_tokens: (a.billable_tokens / 1e6 * 100.0).round() / 100.0,
                billable_tokens: (a.billable_tokens / 1e6 * 100.0).round() / 100.0,
                cost_cents: a.cost_cents,
            })
            .collect();
        sources.sort_by(|a, b| {
            b.billable_tokens
                .partial_cmp(&a.billable_tokens)
                .unwrap_or(std::cmp::Ordering::Equal)
        });

        let mut categories: Vec<CraftCategoryStat> = self
            .craft_categories
            .iter()
            .map(|(category, a)| CraftCategoryStat {
                category: category.clone(),
                call_count: a.call_count,
                error_count: a.error_count,
                completed_count: a.completed_count,
                executing_count: a.executing_count,
            })
            .collect();
        categories.sort_by(|a, b| b.call_count.cmp(&a.call_count));

        let mut status: Vec<CraftStatusStat> = self
            .craft_status
            .iter()
            .map(|(status, count)| CraftStatusStat {
                status: status.clone(),
                call_count: *count,
            })
            .collect();
        status.sort_by(|a, b| b.call_count.cmp(&a.call_count));

        CraftAgentReport {
            tools,
            sources,
            categories,
            status,
            permission: Self::facet(&self.craft_permission),
            thinking: Self::facet(&self.craft_thinking),
        }
    }

    fn facet(items: &HashMap<String, CraftFacetAcc>) -> Vec<CraftSessionFacetStat> {
        let mut v: Vec<CraftSessionFacetStat> = items
            .iter()
            .map(|(value, a)| CraftSessionFacetStat {
                value: value.clone(),
                session_count: a.sessions.len() as u64,
                billable_tokens: (a.billable_tokens / 1e6 * 100.0).round() / 100.0,
                cost_cents: a.cost_cents,
            })
            .collect();
        v.sort_by(|a, b| {
            b.billable_tokens
                .partial_cmp(&a.billable_tokens)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
        v
    }

    fn metrics(&self, delta_tokens: f64, delta_cost: f64) -> Metrics {
        Metrics {
            total_tokens: ((self.input + self.cache + self.output + self.reasoning) / 1e6 * 100.0)
                .round()
                / 100.0,
            input_tokens: (self.input / 1e6 * 100.0).round() / 100.0,
            cache_tokens: (self.cache / 1e6 * 100.0).round() / 100.0,
            output_tokens: (self.output / 1e6 * 100.0).round() / 100.0,
            reasoning_tokens: (self.reasoning / 1e6 * 100.0).round() / 100.0,
            cost: (self.cost * 100.0).round() / 100.0,
            mcp_calls: self.mcp_calls,
            skill_calls: self.skill_calls,
            requests: self.requests,
            sessions: self.sessions.len() as u64,
            delta_tokens,
            delta_cost,
            servers: self.mcp_counts.len() as u64,
            skills: self.skill_counts.len() as u64,
        }
    }
}

/// Percentage change of `cur` vs `prev`, e.g. +20.0 for a 20% increase,
/// rounded to 2 decimals. Returns 0 when there's no baseline to compare.
fn pct_delta(cur: f64, prev: f64) -> f64 {
    if prev <= 0.0 {
        return 0.0;
    }
    ((cur - prev) / prev * 10000.0).round() / 100.0
}

fn clamp_offset(offset: i32) -> i32 {
    offset.min(0)
}

fn add_months(date: chrono::NaiveDate, offset: i32) -> chrono::NaiveDate {
    let total = date.year() * 12 + date.month0() as i32 + offset;
    let year = total.div_euclid(12);
    let month0 = total.rem_euclid(12) as u32;
    chrono::NaiveDate::from_ymd_opt(year, month0 + 1, 1).unwrap()
}

fn iso_date(d: chrono::NaiveDate) -> String {
    d.format("%Y-%m-%d").to_string()
}

fn day_label(target: chrono::NaiveDate, today: chrono::NaiveDate) -> String {
    let diff = (target - today).num_days();
    match diff {
        0 => "今日".to_string(),
        -1 => "昨日".to_string(),
        _ => iso_date(target),
    }
}

fn week_label(
    start: chrono::NaiveDate,
    end: chrono::NaiveDate,
    current_start: chrono::NaiveDate,
) -> String {
    if start == current_start {
        "本周".to_string()
    } else if start == current_start - Duration::days(7) {
        "上周".to_string()
    } else {
        format!(
            "{} ~ {}",
            iso_date(start),
            iso_date(end - Duration::days(1))
        )
    }
}

fn month_label(start: chrono::NaiveDate, current_start: chrono::NaiveDate) -> String {
    if start == current_start {
        "本月".to_string()
    } else if start == add_months(current_start, -1) {
        "上月".to_string()
    } else {
        start.format("%Y-%m").to_string()
    }
}

fn period_window(
    offset: i32,
    label: String,
    start: chrono::NaiveDate,
    end: chrono::NaiveDate,
) -> PeriodWindow {
    PeriodWindow {
        offset,
        label,
        start: iso_date(start),
        end: iso_date(end),
        is_current: offset == 0,
    }
}

// ── Day report: today, 24 hourly buckets ───────────────────────────
fn report_day(events: &[Event], now: DateTime<Local>, offset: i32) -> PeriodReport {
    let offset = clamp_offset(offset);
    let today = now.date_naive();
    let target = today + Duration::days(offset as i64);
    let previous = target - Duration::days(1);
    let mut agg = Agg::default();
    let mut prev = Agg::default();
    let mut buckets = vec![(0.0f64, 0.0f64, 0.0f64, 0.0f64); 24]; // (input, cache, output, reasoning) M
    let mut req_b = vec![0.0f64; 24];
    let mut cost_b = vec![0.0f64; 24];

    for e in events {
        let d = e.ts.date_naive();
        if d == target {
            agg.add(e);
            let h = e.ts.hour() as usize;
            buckets[h].0 += e.input / 1e6;
            buckets[h].1 += e.cache / 1e6;
            buckets[h].2 += e.output / 1e6;
            buckets[h].3 += e.reasoning / 1e6;
            // Match Agg::add exactly: only the request COUNT excludes model-less
            // (slash-command) events; total cost accumulates unconditionally
            // (those events carry cost 0, so this is identical today).
            if !e.model.is_empty() {
                req_b[h] += 1.0;
            }
            cost_b[h] += e.cost;
        } else if d == previous {
            prev.add(e);
        }
    }

    let series = (0..24)
        .map(|h| SeriesPoint {
            // axis ticks every 4h, skipping the 00/24 endpoints
            label: if h % 4 == 0 && h != 0 {
                format!("{:02}", h)
            } else {
                String::new()
            },
            full: format!("{:02}:00", h),
            input: buckets[h].0,
            cache: buckets[h].1,
            output: buckets[h].2,
            reasoning: buckets[h].3,
        })
        .collect();

    PeriodReport {
        window: period_window(
            offset,
            day_label(target, today),
            target,
            target + Duration::days(1),
        ),
        metrics: agg.metrics(
            pct_delta(
                agg.input + agg.cache + agg.output + agg.reasoning,
                prev.input + prev.cache + prev.output + prev.reasoning,
            ),
            pct_delta(agg.cost, prev.cost),
        ),
        series,
        models: agg.models(),
        clients: agg.clients(),
        projects: agg.projects(),
        mcp: Agg::named(&agg.mcp_counts),
        skills: Agg::named(&agg.skill_counts),
        req_trend: req_b,
        cost_trend: cost_b,
        craft: agg.craft(),
        session_metrics: agg.session_metrics(),
    }
}

// ── Week report: current calendar week (Mon-Sun) vs previous week ────
fn report_week(events: &[Event], now: DateTime<Local>, offset: i32) -> PeriodReport {
    let offset = clamp_offset(offset);
    let today = now.date_naive();
    // Monday of the current week (Mon=0 … Sun=6).
    let current_start = today - Duration::days(today.weekday().num_days_from_monday() as i64);
    let start = current_start + Duration::days(offset as i64 * 7);
    let next_start = start + Duration::days(7);
    let prev_start = start - Duration::days(7);

    let mut agg = Agg::default();
    let mut prev = Agg::default();
    let mut buckets = vec![(0.0f64, 0.0f64, 0.0f64, 0.0f64); 7];
    let mut req_b = vec![0.0f64; 7];
    let mut cost_b = vec![0.0f64; 7];

    for e in events {
        let d = e.ts.date_naive();
        if d >= start && d < next_start {
            agg.add(e);
            let idx = (d - start).num_days() as usize;
            if idx < buckets.len() {
                buckets[idx].0 += e.input / 1e6;
                buckets[idx].1 += e.cache / 1e6;
                buckets[idx].2 += e.output / 1e6;
                buckets[idx].3 += e.reasoning / 1e6;
                // Match Agg::add: only the request COUNT excludes model-less
                // events; cost accumulates unconditionally (their cost is 0).
                if !e.model.is_empty() {
                    req_b[idx] += 1.0;
                }
                cost_b[idx] += e.cost;
            }
        } else if d >= prev_start && d < start {
            prev.add(e);
        }
    }

    let weekday = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
    let series = (0..7usize)
        .map(|i| {
            let date = start + Duration::days(i as i64);
            let wd = weekday[i];
            SeriesPoint {
                label: wd.to_string(),
                full: format!(
                    "{} {} {}",
                    wd,
                    MONTHS[(date.month() - 1) as usize],
                    date.day()
                ),
                input: buckets[i].0,
                cache: buckets[i].1,
                output: buckets[i].2,
                reasoning: buckets[i].3,
            }
        })
        .collect();

    PeriodReport {
        window: period_window(
            offset,
            week_label(start, next_start, current_start),
            start,
            next_start,
        ),
        metrics: agg.metrics(
            pct_delta(
                agg.input + agg.cache + agg.output + agg.reasoning,
                prev.input + prev.cache + prev.output + prev.reasoning,
            ),
            pct_delta(agg.cost, prev.cost),
        ),
        series,
        models: agg.models(),
        clients: agg.clients(),
        projects: agg.projects(),
        mcp: Agg::named(&agg.mcp_counts),
        skills: Agg::named(&agg.skill_counts),
        req_trend: req_b,
        cost_trend: cost_b,
        craft: agg.craft(),
        session_metrics: agg.session_metrics(),
    }
}

// ── Month report: current calendar month vs previous calendar month ──
fn report_month(events: &[Event], now: DateTime<Local>, offset: i32) -> PeriodReport {
    let offset = clamp_offset(offset);
    let today = now.date_naive();
    let current_first = chrono::NaiveDate::from_ymd_opt(today.year(), today.month(), 1).unwrap();
    let cur_first = add_months(current_first, offset);
    let next_first = add_months(cur_first, 1);
    let prev_first = add_months(cur_first, -1);
    let days_in_month = (next_first - cur_first).num_days() as usize;

    let mut agg = Agg::default();
    let mut prev = Agg::default();
    let mut buckets = vec![(0.0f64, 0.0f64, 0.0f64, 0.0f64); days_in_month];
    let mut req_b = vec![0.0f64; days_in_month];
    let mut cost_b = vec![0.0f64; days_in_month];

    for e in events {
        let d = e.ts.date_naive();
        if d >= cur_first && d < next_first {
            agg.add(e);
            let idx = (d - cur_first).num_days() as usize;
            if idx < buckets.len() {
                buckets[idx].0 += e.input / 1e6;
                buckets[idx].1 += e.cache / 1e6;
                buckets[idx].2 += e.output / 1e6;
                buckets[idx].3 += e.reasoning / 1e6;
                // Match Agg::add: only the request COUNT excludes model-less
                // events; cost accumulates unconditionally (their cost is 0).
                if !e.model.is_empty() {
                    req_b[idx] += 1.0;
                }
                cost_b[idx] += e.cost;
            }
        } else if d >= prev_first && d < cur_first {
            prev.add(e);
        }
    }

    let series = (0..days_in_month)
        .map(|i| {
            let dn = (i + 1) as u32;
            let label = if i == 0 || dn % 5 == 0 {
                dn.to_string()
            } else {
                String::new()
            };
            SeriesPoint {
                label,
                full: format!("{} {}", MONTHS[(cur_first.month() - 1) as usize], dn),
                input: buckets[i].0,
                cache: buckets[i].1,
                output: buckets[i].2,
                reasoning: buckets[i].3,
            }
        })
        .collect();

    PeriodReport {
        window: period_window(
            offset,
            month_label(cur_first, current_first),
            cur_first,
            next_first,
        ),
        metrics: agg.metrics(
            pct_delta(
                agg.input + agg.cache + agg.output + agg.reasoning,
                prev.input + prev.cache + prev.output + prev.reasoning,
            ),
            pct_delta(agg.cost, prev.cost),
        ),
        series,
        models: agg.models(),
        clients: agg.clients(),
        projects: agg.projects(),
        mcp: Agg::named(&agg.mcp_counts),
        skills: Agg::named(&agg.skill_counts),
        req_trend: req_b,
        cost_trend: cost_b,
        craft: agg.craft(),
        session_metrics: agg.session_metrics(),
    }
}

// ── Heatmap: last ~26 weeks daily totals ────────────────────────────
fn build_heatmap(events: &[Event], today: chrono::NaiveDate) -> Vec<HeatDay> {
    let start = today - Duration::days(25 * 7 + today.weekday().num_days_from_sunday() as i64);
    let mut by_day: HashMap<chrono::NaiveDate, f64> = HashMap::new();
    for e in events {
        let d = e.ts.date_naive();
        if d >= start && d <= today {
            *by_day.entry(d).or_default() += (e.input + e.cache + e.output + e.reasoning) / 1e6;
        }
    }
    let mut days = Vec::new();
    let mut d = start;
    let mut maxv = 0.0f64;
    while d <= today {
        let t = *by_day.get(&d).unwrap_or(&0.0);
        maxv = maxv.max(t);
        days.push((d, t));
        d += Duration::days(1);
    }
    days.into_iter()
        .map(|(date, tokens)| {
            let f = if maxv > 0.0 { tokens / maxv } else { 0.0 };
            let level = if tokens == 0.0 {
                0
            } else if f < 0.25 {
                1
            } else if f < 0.5 {
                2
            } else if f < 0.75 {
                3
            } else {
                4
            };
            HeatDay {
                date: date.format("%Y-%m-%d").to_string(),
                tokens: (tokens * 100.0).round() / 100.0,
                level,
            }
        })
        .collect()
}

#[cfg(test)]
mod tests {
    use super::*;
    use chrono::TimeZone;

    fn event(now: DateTime<Local>, session: &str, model: &str, tool: &str, total: f64) -> Event {
        Event {
            ts: now,
            session: session.to_string(),
            model: model.to_string(),
            project: "test-project".to_string(),
            input: total * 0.5,
            cache: total * 0.25,
            output: total * 0.25,
            reasoning: 0.0,
            cost: total / 1_000_000.0,
            priced: true,
            mcp: Vec::new(),
            skills: Vec::new(),
            tool: tool.to_string(),
            enabled_sources: Vec::new(),
            permission_mode: String::new(),
            thinking_level: String::new(),
            craft_calls: Vec::new(),
        }
    }

    #[test]
    fn client_distribution_groups_usage_by_ai_client() {
        let now = Local.with_ymd_and_hms(2026, 7, 3, 12, 0, 0).unwrap();
        let report = report_day(
            &[
                event(now, "s1", "claude-sonnet", "craft-agent", 1_000_000.0),
                event(now, "s1", "claude-sonnet", "craft-agent", 2_000_000.0),
                event(now, "s2", "claude-sonnet", "claude-code", 4_000_000.0),
                // Model-less events are slash-command/tool metadata; they must
                // not inflate request counts or client usage distribution.
                event(now, "s3", "", "craft-agent", 8_000_000.0),
            ],
            now,
            0,
        );

        assert_eq!(report.metrics.requests, 3);
        assert_eq!(report.clients.len(), 2);

        let claude = &report.clients[0];
        assert_eq!(claude.id, "claude-code");
        assert_eq!(claude.label, "Claude Code");
        assert_eq!(claude.total_tokens, 4.0);
        assert_eq!(claude.requests, 1);
        assert_eq!(claude.sessions, 1);
        assert!(claude.priced);

        let craft = &report.clients[1];
        assert_eq!(craft.id, "craft-agent");
        assert_eq!(craft.label, "Craft Agents");
        assert_eq!(craft.total_tokens, 3.0);
        assert_eq!(craft.requests, 2);
        assert_eq!(craft.sessions, 1);
        assert!(craft.priced);
    }

    #[test]
    fn period_offsets_select_previous_windows() {
        let now = Local.with_ymd_and_hms(2026, 7, 3, 12, 0, 0).unwrap();
        let yesterday = Local.with_ymd_and_hms(2026, 7, 2, 12, 0, 0).unwrap();
        let last_week = Local.with_ymd_and_hms(2026, 6, 25, 12, 0, 0).unwrap();
        let last_month = Local.with_ymd_and_hms(2026, 6, 15, 12, 0, 0).unwrap();
        let events = vec![
            event(now, "today", "claude-sonnet", "claude-code", 1_000_000.0),
            event(
                yesterday,
                "yesterday",
                "claude-sonnet",
                "claude-code",
                2_000_000.0,
            ),
            event(
                last_week,
                "last-week",
                "claude-sonnet",
                "claude-code",
                3_000_000.0,
            ),
            event(
                last_month,
                "last-month",
                "claude-sonnet",
                "claude-code",
                4_000_000.0,
            ),
        ];

        let day = report_day(&events, now, -1);
        assert_eq!(day.window.label, "昨日");
        assert_eq!(day.window.start, "2026-07-02");
        assert_eq!(day.metrics.total_tokens, 2.0);

        let week = report_week(&events, now, -1);
        assert_eq!(week.window.label, "上周");
        assert_eq!(week.window.start, "2026-06-22");
        assert_eq!(week.window.end, "2026-06-29");
        assert_eq!(week.metrics.total_tokens, 3.0);

        let month = report_month(&events, now, -1);
        assert_eq!(month.window.label, "上月");
        assert_eq!(month.window.start, "2026-06-01");
        assert_eq!(month.window.end, "2026-07-01");
        assert_eq!(month.metrics.total_tokens, 7.0);
    }
}
