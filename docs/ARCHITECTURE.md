...
Git 仓库只保存源码、配置、文档和 workflow。`.app`、`.dmg`、installer、archive 与 checksum 是 **GitHub Releases assets**，不进入 Git 历史。发布流程由 `.github/workflows/release.yml` 驱动，tag `v*.*.*` 后在 macOS runner 构建并上传 DMG。详细策略见 `docs/RELEASE.md`。
