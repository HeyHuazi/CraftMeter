    ...
    <div>
      <div style={{ position: "relative", height, display: "flex", alignItems: "flex-end", gap: `${gapPct}%` }}>
        ...
        {data.map((d, i) => {
          ...
          const hO = ((d.output + d.reasoning) / max) * height, hI = ((d.input + d.cache) / max) * height;
      ...
      {hi && (
        <div style={{
          ...
          <span style={{ color: accent, fontWeight: 600 }}>
            {(() => { const tot = hi.input + hi.cache + hi.output + hi.reasoning; return tot === 0 ? "无 Token" : fmtTokens(tot) + " Token"; })()}
          </span>
    ...
    <div ref={wrapRef} style={{ position: "relative" }}>
      ...
      {hi && (
        <div style={{
          ...
          font: `500 10px ${t.mono}`, whiteSpace: "nowrap", pointerEvents: "none", zIndex: 9999,
          boxShadow: "0 4px 14px rgba(0,0,0,0.35)" }}>
          <span style={{ color: accent, fontWeight: 600 }}>{hi.tokens === 0 ? "无调用" : fmtTokens(hi.tokens) + " Token"}</span>
