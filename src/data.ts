...
export interface ClientUsageStat { id: string; label: string; totalTokens: number; inputTokens: number; cacheTokens: number; outputTokens: number; reasoningTokens: number; cost: number; requests: number; sessions: number; priced: boolean }
export interface NamedCount { name: string; count: number }
export interface ProjectStat { name: string; totalTokens: number; cost: number; requests: number; sessions: number; priced: boolean }
export interface SessionMetrics { durationSeconds: number; activeSeconds: number; messageCount: number; userMessageCount: number }
export interface CraftToolStat { name: string; displayName: string; category: string; callCount: number; errorCount: number; completedCount: number; executingCount: number }
export interface CraftSourceStat { slug: string; sessionCount: number; totalTokens: number; billableTokens: number; costCents: number }
export interface CraftCategoryStat { category: string; callCount: number; errorCount: number; completedCount: number; executingCount: number }
export interface CraftStatusStat { status: string; callCount: number }
export interface CraftSessionFacetStat { value: string; sessionCount: number; billableTokens: number; costCents: number }
export interface CraftAgentReport {
  tools: CraftToolStat[];
  sources: CraftSourceStat[];
  categories: CraftCategoryStat[];
  status: CraftStatusStat[];
  permission: CraftSessionFacetStat[];
  thinking: CraftSessionFacetStat[];
}
export interface Metrics {
  totalTokens: number; inputTokens: number; cacheTokens: number; outputTokens: number; reasoningTokens: number; cost: number;
...
export interface PeriodWindow { offset: number; label: string; start: string; end: string; isCurrent: boolean }
export interface PeriodReport {
  window: PeriodWindow;
  metrics: Metrics; series: SeriesPoint[]; models: ModelStat[]; clients: ClientUsageStat[];
  projects: ProjectStat[]; mcp: NamedCount[]; skills: NamedCount[]; reqTrend: number[]; costTrend: number[];
  craft: CraftAgentReport; sessionMetrics: SessionMetrics;
}
...
export interface PeriodOffsets { day: number; week: number; month: number }

export async function fetchDashboard(offsets: PeriodOffsets = { day: 0, week: 0, month: 0 }): Promise<Dashboard> {
  const inTauri = typeof window !== "undefined" && "__TAURI_INTERNALS__" in window;
  if (inTauri) return await invoke<Dashboard>("get_dashboard", {
    dayOffset: offsets.day,
    weekOffset: offsets.week,
    monthOffset: offsets.month,
  });
