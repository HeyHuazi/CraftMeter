...
AppViewModel.swift: executable 运行时状态与 coordinator 组合根；完整 Usage Analytics 与菜单栏历史摘要 runtime 均按首次对应请求懒构造，避免启动同步恢复大缓存或初始化定价目录；向 ProviderFactory 与 Relay 浏览器导入协调器注入同一凭据发现服务，生产检查 Applications 中最高已安装版本，Debug 测试仅使用显式版本隔离宿主机状态；新启动持久化逻辑不得继续回填到此类。
