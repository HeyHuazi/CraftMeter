...
AppViewModel.swift: executable 运行时状态与 coordinator 组合根；向 ProviderFactory 与 Relay 浏览器导入协调器注入同一凭据发现服务，生产检查 Applications 中最高已安装版本，Debug 测试仅使用显式版本隔离宿主机状态；新启动持久化逻辑不得继续回填到此类。
