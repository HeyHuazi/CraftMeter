...
BrowserCredentialService.swift: 统一发现浏览器 Cookie、命名 Cookie 与 Bearer 候选；`.background` 在读取交互缓存、枚举文件或访问 Safe Storage 前立即拒绝，只有显式 `.interactiveImport` 可实时读取。
KimiBrowserCookieService.swift: Kimi 浏览器 Cookie/localStorage 检测边界；所有实时 API 强制携带 `BrowserCredentialAccessIntent`，后台 intent 零文件/Keychain 访问。
RelayBrowserImportCoordinator.swift: 对 Relay 新增/编辑执行无副作用浏览器凭据预检，输出脱敏来源、User ID 缺失和验证下一步，不写配置或 Keychain。
RelayCurlImportParser.swift: 不执行 shell 的有限 cURL tokenizer；只接受 HTTP/HTTPS `/api/user/self`，提取 Bearer、Cookie、`New-Api-User` 与站点 origin，解析错误不得包含原始秘密。
RelayCurlImportCoordinator.swift: 以 Generic NewAPI manifest/HTTP/响应解释器验证 cURL 候选；Bearer 必须配 User ID，Cookie 可从 self 响应补全身份，且只返回单一可持久化凭据与脱敏展示结果。
KeychainAccessPolicy.swift: 统一构造 Keychain 认证属性；后台同时使用 `LAContext.interactionNotAllowed` 与 `kSecUseAuthenticationUIFail`，并仅按凭据类别、操作和 OSStatus 输出脱敏失败审计。
KeychainGenericPasswordWriter.swift: 统一 Security.framework generic-password 的严格 update-or-add 状态机；仅 `errSecItemNotFound` 允许新增，权限或交互错误绝不删除旧项。
KeychainService.swift: `craftmeter` 进程门禁凭据 vault；启动/后台读写在显式用户解锁前为纯内存零 Security 调用，解锁 single-flight 且失败后进程级熔断，历史迁移仅在显式准备事务内执行，同值凭据零写入且失败不污染旧状态。
KeychainService+LiveSecureStore.swift: CraftMeter vault 的 Security.framework live adapter；read/read-all/update/add/delete 均复用统一认证策略与脱敏失败审计。
KeychainService+UsageCredentialStore.swift: 将 Infrastructure `UsageCredentialStore` 的 Domain provider identity 映射到 CraftMeter 默认 service account；协议适配不拥有底层 Keychain 策略。
SecurityCredentialReader.swift: 仅供显式 OAuth/浏览器导入及账户切换使用的外部 generic-password 边界；非交互读写同时硬禁止认证 UI，写入严格 update-or-add，XCTest 默认短路。
...
- cURL 导入只解析白名单，不执行命令；原始 cURL、Bearer 与 Cookie 仅存在于同步/异步调用栈，禁止进入 SwiftUI 可观察状态、配置、日志或诊断消息。
- cURL 验证必须选择保存后可独立复现的单一认证方式；禁止用 Bearer+Cookie 临时组合验证后只持久化其中一半。
- `forceRefresh` 只刷新网络/数据缓存，绝不授予外部 Keychain 或浏览器实时访问；后台 Provider 只能消费当前进程已解锁的 CraftMeter vault、文件和环境变量。
- CraftMeter vault 的后台零交互由“显式解锁前零 Security.framework 调用”保证，而非只依赖 `kSecUseAuthenticationUIFail`；持久 prepared flag 不能代表新进程或新签名身份已授权。
- `SecurityCredentialReader` 只能被显式 OAuth 导入、浏览器解密链路和用户账户切换调用；后台 Provider 禁止直接依赖。
